...
- Meat finished labels: `MeatFinishedLabelDialog.tsx` reuses saved Label Maker templates and `/api/v1/labelmaker/render-pdf` with an explicit `template_id` and batch production date. `/labelmaker/templates?inv_item_id=...` finds templates whose production product links that output SKU directly or through forming pack configuration; other templates are excluded. Users select the output and saved label; an exact pack match or a sole candidate is preselected. Saved ingredient/nutrition/storage/pack weight remain unchanged; expiry follows the saved shelf life. Existing unfiltered template queries and forming/hot label rendering retain their behavior. The finished-label button shares the carton button's unsaved/historical/voided guards and remains available after QA/stock entry. No schema migration is needed.
